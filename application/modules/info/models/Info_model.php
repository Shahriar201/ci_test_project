...
class Info_model extends CI_Model
{
	...
	{
		$info = $this->db->get_where('add_info', ['id' => $id])->row();
		...
		// $delete = $this->db->where("id", $id)->delete("add_info");
		$delete = $this->db->delete("add_info", ['id' => $id]);

		if ($delete) {
			// delete image
			if(!empty($info->image)){
				unlink('assets/image/' . $info->image);
			}
	...
	public function view_info($id){
		return $this->db->get_where('add_info', ['id' => $id])->row();
